...
%DIF DEL overleaf/main.tex   Thu Mar  5 09:29:36 2026
%DIF ADD draft/working.tex   Wed Mar 11 13:23:51 2026
...
\begin{document}
    ...
    % Spatial heterogeneity can modify mass, energy, momentum fluxes \citep{philip1959theory}.
    % Horizontal advection cal alter local rate of ET and surface energy balance. Assumption of constant stomatal resistance predicted ET would be high at the wet/dry interface \cite{lang_influence_1974}

\DIFdelbegin \DIFdel{Advection -- the horizontal transport of atmospheric properties -- has been a subject of longstanding interest across diverse fields.
Wind advectionoften transports air from one type of surface to another, for example, from a fallow area to an irrigated field.
Advection from can increase evapotranspiration in fields  downwind of fallowed or barren land areas, enhance }\DIFdelend \DIFaddbegin \DIFadd{The surface energy balance and the associated exchange of water vapor between the land surface and the atmosphere are routinely modelled under the assumption of horizontal homogeneity, yet most land surfaces are spatially heterogeneous.
...
Its effects have been documented in a wide range of environments, including elevated evapotranspiration (ET) in irrigated fields bordered by dry land \mbox{%DIFAUXCMD
\citep{rao1974local}}\hskip0pt%DIFAUXCMD
, enhanced }\DIFaddend evaporation from lakes and reservoirs \citep{higgins2013measured}, and \DIFdelbegin \DIFdel{accelerate ice melting in snow-covered soils \mbox{%DIFAUXCMD
\cite{shook1997snowmelt, essery2006boundary}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{accelerated snowmelt at exposed patch margins \mbox{%DIFAUXCMD
\citep{shook1997snowmelt, essery2006boundary}}\hskip0pt%DIFAUXCMD
}\DIFaddend .

     In some cases, sensible heat may be drawn from the overlying air to support evaporation, so that the latent heat flux may exceed the local net radiation (evaporative enhancement) \citep{yang2013new, kool_within-field_2018}.  
     This occurs because warm, dry air moving over a cooler, well-watered surface provides an additional source of thermal energy that supplements the energy provided by net radiation \cite{spronken2000advection}.

    % How surface controls (e.g., finite resistance / surface relative humidity) shape the partitioning rather than assuming a perfectly saturated surface everywhere.
    While early analytical work often assumed a perfectly saturated surface \cite{lang1974influence, rao1974local}, more recent research has focused on how surface controls, such as stomatal resistance and surface relative humidity, shape energy partitioning within a soil-plant-atmosphere continuum \cite{baldocchi1995intra}. 
     % Surface controls -- such as finite surface resistance and surface relative humidity -- then regulate the energy partitioning, rather than assuming a perfectly saturated surface everywhere. 

    Horizontal transport of heat and moisture across sharp surface contrasts can therefore modify near-surface gradients and, in turn, the surface turbulent fluxes.

    
    %  energy balance is sensitive to advection,
    Over heterogeneous surfaces, advection transports heat and water vapor, perturbing the one-dimensional surface energy balance assumed by most ET models.

    The impact of water vapor advection is most evident near abrupt changes in land conditions, such as the edges of irrigated fields or water bodies \cite{rao1974local, higgins2013measured}.

    
    \textbf{Advection and water resources}

    
    Quantifying water losses due to advection is important for water-resources planning, particularly for irrigation management in heterogeneous landscapes. Advective enhancement can be substantial, especially near leading edges where hot, dry air from adjacent fallow or barren areas increases evaporative demand over irrigated patches. Lysimeter studies show that advection can raise ET by roughly 30–10\% above what local net radiation alone could support \cite{rijks1971water, rosenberg1978extreme, spronken2000advection}. 

    Consistent with this, advective effects cannot be neglected over mosaic irrigated–fallow land surfaces.
    This is particularly consequential during early growth stages, when seedlings are most vulnerable to high temperatures and elevated vapor pressure deficits.

    
    Quantifying water losses due to advection is needed to effectively manage water resources.
    Advection is relevant to water resources planning, particularly for irrigation management. 
    Advective enhancement is significant, especially leading edge effects.  For example, lysimeter studies have shown that advection can increase ET on the order of 30–100\%  higher than what local net radiation alone could support \cite{rijks1971water, rosenberg1978extreme, spronken2000advection}.
     Advective effects cannot be neglected over such variable land surfaces as mosaic irrigated/fallow lands.
    Quantifying advective effects is important for irrigation management.
    During early growth stages, seedlings are particularly susceptible to heat and high vapor pressure deficits.

    % TODO: Move to historical: 
    \citet{rosenberg1978extreme} documented record-breaking evapotranspiration rates in an irrigated alfalfa field during a severe drought in the midwestern United States.  On clear mid-summer days, net radiation  could support at most $\sim$7 mm /d of ET, and sensible heat advection increased evapotranspiration to 10–14 mm/d.
    Allen et al (2011) observation in an irrigated agricultural area surrounded by desert, where lysimeter measurements recorded actual evapotranspiration  rates that significantly exceeded the energy supplied by local net radiation. In some instances, the ratio of ET/Rn  was greater than 2 meaning the actual water loss was over 100\% higher than what local solar radiation alone could support. 
    \textit{dry air from the surrounding desert provides the additional thermal energy required to drive these extreme evaporation rates. This study demonstrates that assuming net radiation is the only source of evaporative energy can lead to significant underestimation of ET in arid and semi-arid regions.}

    %  Oke (1979) observed suburban lawns where daily evaporation from mini-lysimeters exceeded net radiation.

    
    \textbf{Land fallowing}
    Trend towards increasing land fallowing  \cite{melton2014mapping}.
    % See Pancorbo
    During California drought from 2011 - 2015, for example, area of land fallowed increased from x to y?. 

    Previous studies examined some aspects of land use changes, driving factors, and impacts on groundwater during 2007–2016 in Central Valley. During this period, increases in fallow land area and reductions in irrigated land were reported [8,16,17]. 
    % 8 Medellín-Azuara, J.; MacEwan, D.; Howitt, R.E.; Sumner, D.A.; Lund, J.R.; Scheer, J.; Gailey, R.; Hart, Q.; Alexander, N.D.; Arnold, B.; et al. Economic Analysis of the 2016. California Drought on Agriculture; Center for Watershed Sciences: Davis, CA, USA, 2016.
    % Cooley, H.; Donnelly, K.; Phurisamban, R.; Subramanian, M. \textit{Impacts of California’s ongoing Drought: Agriculture}; Pacific Institute: Oakland, CA, USA, 2015; p. 24.
    % [17] Medellin-Azuara, J.; MacEwan, D.; Howitt, R.E.; Koruakos, G.; Dogrul, E.C.; Brush, C.F.; Kadir, T.N.; Harter, T.; Melton, F.; Lund, J.R. Hydro-economic analysis of groundwater pumping for irrigated agriculture in California’s Central Valley, USA. \textit{Hydrogeol. J.} 2015, \textit{23}, 1205–1216.

    
    Tools to address such questions as - how much water is saved by fallowing land in irrigated agricultural lands? 
    Are savings to some extent offset by advection? 
    How sensitive are things to spatial pattern?

    \paragraph{Advection and remote sensing}
    \medskip 

    \textit{Quantifying advection is also important for improving the accuracy of remote-sensing ET models. Although remote sensing advances now enable routine ET mapping, most pixel-based approaches treat each pixel as vertically one-dimensional and therefore neglect horizontal transport of heat and moisture \cite{liou2014evapotranspiration, mcshane2017review}. This limitation is especially problematic in advective settings, such as agricultural landscapes with patchworks of irrigated and fallow fields \cite{derardja2024advancements}. A small number of recent studies incorporate advection explicitly in remote-sensing ET algorithms or use high-resolution thermal imagery to diagnose advective fluxes, including modifications to SEB and SEBAL \cite{liu2016regional, mkhwanazi2015sebal, wang2024advection}; we discuss these in more detail in Section~\ref{sec:discussion}.
    }
    Quantifying advection also needed to improve accuracy of ET models.
    % Connection to remote sensing, why topic is timely
    Remote sensing advances have enabled mapping ET; however, most models treat pixels as vertically one-dimensional, neglecting transport of heat and moisture \cite{liou2014evapotranspiration, mcshane2017review}.
    As a result, advective environments -- such as agricultural lands with a patchwork of fallow and irrigated fields  -- are a challenge for ET models \cite{derardja2024advancements}.

    Remote-sensing ET models typically do not account for advection. Exceptions are several recent studies that incorporate advection into remote-sensing ET models  \cite{mkhwanazi2015sebal, liu2016regional},  including modifications to the SEB  \cite{liu2016regional}  and  SEBAL \cite{mkhwanazi2015sebal}; discussed in more detail in Section \ref{sec:discussion}.

    Other studies have used high-resolution thermal imagery to diagnose advective fluxes \cite{wang2024advection}.

    
    \paragraph{Upscale in heterogeneous landscapes}

    Advection poses an ongoing challenge in connecting high-resolution land surface data with the coarse-resolution variables used in atmospheric models \cite{kustas_effects_2003}.
    Predicting how the whole differs from ths sum of its parts involves understanding how patches `talk' to one another.

    
     The magnitude of advective enhancement of latent heat transfer depends on the background conditions, patch configuration, and stomatal feedbacks to the advected humidity-deficit \cite{raupach_averaging_1982}(Raupach, 1991; Klaassen, 1992; Kroon and de Bruin, 1993). 

    
    % Hardner: The differences in surface energetics between snow-covered and snow-free areas lead to a heterogeneous distribution of surface temperature and near-surface water vapour. These horizontal gradients drive a lateral exchange of heat (sensible heat advection) and water vapour (latent heat advection when considering the induced condensation or sublimation) over the leading edge of a snow patch. 

    \paragraph{Advances in remote sensing}

    This topic is particularly timely, as advances in space-based satellites, aircraft, drones, and infrared cameras have the potential to observe and analyze advective processes across various scales and conditions \cite{volk2024assessing, fisher2020ecostress, derardja2024advancements}.   
    \textit{Look for studies advancing measurement of advective effects, diagnosis using thermal imagery, drones, model/RS combination? See Kustas.}
    Studies using RS/aerial imagery to diagnose turbulent heat flux and advective effects...

    % ASTER (Advanced Spaceborne Thermal Emission and Reflection Radiometer) on NASA’s Terra provides $\sim$90 m thermal infrared imagery (with 15–30 m VNIR/SWIR) that can resolve field-scale land-surface temperature gradients useful for diagnosing advective edge effects.
    % IKONOS

    Remote sensing technologies thus remain under-leveraged in quantifying advection and its impacts on water resources  \cite{liu2016regional}. 
    To leverage ongoing advances in remote sensing, modeling frameworks must accommodate complex, spatially variable boundary conditions derived from remote sensing,
    to describe advection impacts at the large scales.
    % We want the capabilities to handle complex boundary conditions measured from remote sensing.

    \paragraph{Revisiting Sutton's problem}

    Here, we revisit Sutton's solution \cite{sutton1934wind} 
    -- describing enhanced evaporation downwind of a dry/wet interface -- 
    and outline an approach to bridge remote sensing. 

    
    
    Application explored is specific to mosaic fallow and cultivated areas, where highly advective conditions, experiments have demonstrated impacts. 

    
    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{figures/Figure7.1.png}
        \caption{Sutton's problem. Definition sketch of two-dimensional internal boundary layer problem. \textit{Placeholder}.
        Sutton's problem describes the idealized system of  air advection across a sharp transition in surface moisture,  from an upwind dry, water-limited surface to a downwind wet or irrigated surface. Upwind, strong sensible heating produces a warm, dry boundary layer.  Where the surface switches abruptly to a wet, energy-limited regime.  As the  dry air passes over this irrigated patch, an internal boundary layer develops and grows with distance from the transition, within which temperature and humidity adjust toward values compatible with the wet surface fluxes, while the overlying flow retains the properties of the upwind dry boundary layer. }
        \label{fig:7.1}
    \end{figure}

    \begin{figure}
        \centering
        \includegraphics[width=0.8t\linewidth]{figures/Figure7.3.png}
        \caption{Internal boundary layer of specific humidity in a dynamically uniform boundary layer.}
        \label{fig:placeholder}
    \end{figure}

    
    The study first provides a brief overview of the first principles involved in the problem, including radiation balance, energy balance,  mean momentum balance, and mean mass balance of water vapor.

    We then examine the second principles, including the assumption of a gradient diffusion representation for turbulent fluxes.
    We discuss the limitation of this representation, 
    and show that it still yields plausible results when analyzing contrasts, compared to more complex formulations \cite{rao1974local}.

    Following a review of prior work / brief history. 

    Three sections to the study:
    (1) comparison to experimental results (Rider, Baldocchi) – showing good agreement.
    (2) estimate of fallowing effects on ET, comparing both to continuous cultivation,
    (3) scenario exploration.

    This work advances on previous work by explicitly considering spatial configuration and wind. 
...
% Brutsaert: The solution describes evaporation from a uniformly-moist surface of limited size, such as a lake or an irrigated field, whose temperature and roughness are uniform and the same as the surrounding presumably drier land surface. Under these simple conditions, the water vapor is a passive admixture that does not affect the dynamics of the motion or the stability. Hence, the problem can be considered non-dynamic and non-energetic and only the passive transfer of water vapor is of concern. This means that the wind profile fI = fI(z), v = w = 0, remains unchanged as the air travels past the humidity discontinuity
    ...
    Lastly, Sutton’s solution neglected evaporation from the dry land upwind of the wet area, and Philips approach  included an upwind evapotranspiration rate, which made the model more broadly applicable.
    ...
    % TODO: check Rosenberg et al., 1983.
    ...
    \subsection*{1970s onwards: Higher-order closure}
     ...
     Th 1970s saw the application of higher-order closure models to address the advection problem, which addressed these limitations. 
...
\subsubsection{Ground heat flux}  
    ...
    Why can't we just adjust $alpha\_{PT}$ upwards? 
     In arid or semi-arid environments where advection is prevalent, $\alpha\_{PT}$ values as high as 1.4, 1.5, or even 1.55 observed, reflecting the increased evaporative demand caused by the desert-like surroundings. (cite Diaz-Espejo et al., 2005; Li and Yu, 2007).
    This approach used in Sebal-A paper.
    Doesn't account for spatial patter, leading edge. How are we going to parameterize $\alpha\_{PT}$? More reason to use openET model output. 
